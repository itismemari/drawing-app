...
export default function FileDropzoneVideo({ ...props }) {
  const videoref = useRef<HTMLDivElement>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [storedVideo, setStoredVideo] = useState<File | null>(null);
  ...
  const handleDragOver = (e: React.DragEvent<HTMLDivElement>) => {
    e.preventDefault();
    videoref.current?.classList.add("border-indigo-600");
  };

  const handleDragLeave = (e: React.DragEvent<HTMLDivElement>) => {
    e.preventDefault();
    videoref.current?.classList.remove("border-indigo-600");
  };
  ...
  const handleConfirm = async (e: React.FormEvent) => {
    ...
    formData.append("storedVideo", storedVideo);

    const res = await fetch("/api/storedVideo", {
      method: "POST",
      body: formData,
    });
        ...
        {!preview && (
          <div className="text-center">
            <img
              ...
              src="/video-upload-svgrepo-com.svg"
              alt="Upload icon"
            />
