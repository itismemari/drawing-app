...
export default function Home() {
  ...
  const saveCanvas = async () => {
    ...
    // Let React flush the re-render before taking screenshot
    requestAnimationFrame(async () => {
      const screen = document.getElementById("canvas") as HTMLElement;
      const canv = await html2canvas(screen);
      const dataURL = canv.toDataURL("image/png");
      const link = document.createElement("a");
      link.href = dataURL;
      link.download = "canvas-with-cards.png";
      link.click();
